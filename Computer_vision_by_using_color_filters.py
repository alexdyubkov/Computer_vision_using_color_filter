##############################################################################часть 1 - создание маски##############################################################################
#Идея
# 1)Reading RGB image --> 2)Converting to HSV --> 3)Implementing Mask -->
# -->4)Finding Contour Points -->5) Extracting Rectangle Coordinates -->
# --> 6Drawing Bounding Box --> Putting Label
#
# Result:
...
#создаем окно "Track Bars", даем возможность менять размеры окна когда используем WINDOW_NORMAL
cv2.namedWindow('Track Bars', cv2.WINDOW_NORMAL)
 
#определяем Track Bars(они помогут найти маску)
#For minimum range
cv2.createTrackbar('min_blue', 'Track Bars', 0, 255, do_nothing) #"min_blue" -имя Track bar; "Track Bars"-имя окна; do_nothing - ф-ция 
...
#imread() -читаем изображение через opencv  ->он открывает изображение как numpy array ->но он открывает как BGR
...
# посмотреть исходное изображнеие
# Giving name to the window with Original Image
# And specifying that window is resizable
cv2.namedWindow('Original Image', cv2.WINDOW_NORMAL) 
cv2.imshow('Original Image', image_BGR) #imshow() - открыть изображение в окне original image как BGR
...
# #цикл, кот позволяет не закрывать track bars. Его можно было включать, чтобы по кускам проверить код выше.
# while True:
#     if cv2.waitKey(0):
#         breaks


# С помощью Track bar находим маску
while True:
    # выбираем 6 цветов
    # For minimum range
...
"""
##############################################################################найти объект в реальном времени по маске##############################################################################
...
# Defining lower bounds and upper bounds of founded Mask(вставляем рез части 1)
...
camera = cv2.VideoCapture(0)#0 - встроенная камера, можешь попробовать 1
...
while True:
    ...
    if v == '3':
        _, contours, _ = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)  #_ =модиф изобр ; contours = найденные контуры
    ...
    if contours:
        #если нашли контуры, то нах коорд контура
        ...
        #рисуем  рамку по координатам +15 смещение на всякий
        # Drawing Bounding Box on the current BGR frame
        cv2.rectangle(frame_BGR, (x_min - 15, y_min - 15), #верх лев точк
                      (x_min + box_width + 15, y_min + box_height + 15), #прав ниж точка
                      (0, 255, 0), 3)  #цвет рамки
